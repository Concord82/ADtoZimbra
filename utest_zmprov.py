...
class ZimbraClientTest(unittest.TestCase):
	...
	def test_01_getalldomain(self):
		...
		self.assertEqual(getalldomain, ['zimbra-mailserver.internal.cons.tsk.ru', 'test.cons.tsk.ru'])
	...
	def test_02_getAlldistributionLists(self):
		...
		self.assertIn('cons-talks@zimbra-mailserver.internal.cons.tsk.ru', AllDistribList)
		
	def test_03_createDistributionList(self):
		''' Создаем список рассылки'''
		print("id: " + self.id())
		self.assertNotEqual(self.zm_exec.createDistributionList('test2@test.cons.tsk.ru'), False)
	...
	def test_05_addDistributionListMember(self):
		...
		self.assertTrue( self.zm_exec.addDistributionListMember('cons-talks@zimbra-mailserver.internal.cons.tsk.ru', user_list) )
			
	def test_06_getDistributionListmembership(self):
		'''получаем членов списка рассылки '''
		print("id: " + self.id())
		DistributionListmembership = self.zm_exec.getDistributionListmembership('cons-talks@zimbra-mailserver.internal.cons.tsk.ru')
		flagg = True
		for i in range(10):
	...
	def test_08_removeDistributionListMember(self):
		...
		for i in range(10):
			user_list.append('user'+str(i)+'@test.cons.tsk.ru')
		self.assertTrue( self.zm_exec.removeDistributionListMember('cons-talks@zimbra-mailserver.internal.cons.tsk.ru', user_list) )
	...
	def test_13_deleteDistributionList(self):
		...
		print("id: " + self.id())
		self.assertTrue(self.zm_exec.deleteDistributionList('test2@test.cons.tsk.ru'))
		
	def test_14_deleteDistributionList(self):
		''' Удаляем список рассылки повторно'''
		print("id: " + self.id())
		self.assertFalse(self.zm_exec.deleteDistributionList('test2@test.cons.tsk.ru'))
